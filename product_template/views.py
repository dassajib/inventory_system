...
def viewProduct(request, pk):
    product = ProductTemplate.objects.get(id=pk)
...
def showAllIngredients(request):
    ingredients = IngredientTemplate.objects.all()
    serializer = IngredientSerializer(ingredients, many=True)
    return Response(serializer.data)

@api_view(['GET'])
def viewIngredient(request, pk):
    ingredient = IngredientTemplate.objects.get(id=pk)
...
@api_view(['GET'])
def deleteIgredient(request, pk):
    ingredient = IngredientTemplate.objects.get(id=pk)
    ingredient.delete()
